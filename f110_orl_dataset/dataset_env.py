...
class F1tenthDatasetEnv(gym.Env):
    ...
    def get_dataset(
        ...
        if self.max_trajectories is None:
            ...
            max_traj = min(max_traj, self.max_trajectories)

        for trajectory in range(max_traj+1):
            # print(trajectory)
            ...
            if self.flatten_acts:
                data_dict['actions'] = np.vstack((root[trajectory]['actions']['steering'][:], root[trajectory]['actions']['velocity'][:]))
            else:
            ...
            if self.laser_obs:
                data_dict['scan'] = root[trajectory]['observations']['scan'][:,::self.subsample_laser]
